...
;; URL: https://github.com/syl20bnr/spacemacs/blob/develop/layers/%2Bdistributions/spacemacs-bootstrap/local/evil-evilified-state/evil-evilified-state.el
...
;; This code is adapted from https://github.com/syl20bnr/spacemacs/blob/develop/layers/%2Bdistributions/spacemacs-bootstrap/local/evil-evilified-state/evil-evilified-state.el
...
(defalias 'evilified-state-exit 'evilified-state--evilified-state-on-exit)
...
(add-hook 'evilified-state-entry-hook
          'evilified-state--evilified-state-on-entry)

;; default key bindings for all evilified buffers
(define-key evilified-state-map "/" 'evil-search-forward)
(define-key evilified-state-map ":" 'evil-ex)
(define-key evilified-state-map "h" 'evil-backward-char)
(define-key evilified-state-map "j" 'evil-next-visual-line)
(define-key evilified-state-map "k" 'evil-previous-visual-line)
(define-key evilified-state-map "l" 'evil-forward-char)
(define-key evilified-state-map "n" 'evil-search-next)
(define-key evilified-state-map "N" 'evil-search-previous)
(define-key evilified-state-map "v" 'evil-visual-char)
(define-key evilified-state-map "V" 'evil-visual-line)
(define-key evilified-state-map "gg" 'evil-goto-first-line)
(define-key evilified-state-map "G" 'evil-goto-line)
(define-key evilified-state-map (kbd "C-f") 'evil-scroll-page-down)
(define-key evilified-state-map (kbd "C-b") 'evil-scroll-page-up)
(define-key evilified-state-map (kbd "C-e") 'evil-scroll-line-down)
(define-key evilified-state-map (kbd "C-y") 'evil-scroll-line-up)
(define-key evilified-state-map (kbd "C-d") 'evil-scroll-down)
(define-key evilified-state-map (kbd "C-u") 'evil-scroll-up)
(define-key evilified-state-map (kbd "C-o") 'evil-jump-backward)
(cond
 ((display-graphic-p)
  ;; In GUI Emacs, use <C-i>, which can be distinguished from <tab>, to avoid
  ;; shadowing any TAB key bindings.
  (define-key evilified-state-map (kbd "<C-i>") 'evil-jump-forward))
 (evil-want-C-i-jump
  ;; In terminal mode, and only if explicitly requested by the user, use C-i,
  ;; which shadows TAB key bindings.
  (define-key evilified-state-map (kbd "C-i") 'evil-jump-forward)))
(define-key evilified-state-map (kbd "C-z") 'evil-emacs-state)
(define-key evilified-state-map (kbd "C-w") 'evil-window-map)
(setq evilified-state-map-original (copy-keymap evilified-state-map))
