...
*** ⭐🏢 [[./editor/tempel][tempel]]: snippets using [[https://github.com/minad/tempel][tempel]]; alternative to Doom's ~:editor snippets~ (which currently uses yasnippet)
I've added some hacky/invasive extensions to tempel. I also add custom template elements and (with [[doom-module::completion vertico][:completion vertico]] enabled) integrate my [[https://github.com/AjaiKN/consult-tempel][consult-tempel]] package.
*** 🚧 [[./editor/things][things]]: evil text objects based on [[https://github.com/noctuid/things.el][things.el]]
...
*** [[./emacs/calc][calc]]: tweaks for the built-in [[https://www.gnu.org/software/emacs/manual/html_mono/calc.html][calc]]
...
*** 🏢 [[./emacs/tramp-extra][tramp-extra]]: performance tweaks and bugfixes for [[https://www.gnu.org/software/emacs/manual/html_node/tramp/index.html][TRAMP]]
Flags:
**** +hlo: use [[https://github.com/jsadusk/tramp-hlo][tramp-hlo]] for further performance improvements
**** +rpc: use [[https://github.com/ArthurHeymans/emacs-tramp-rpc][tramp-rpc]] for further performance improvements
...
** :lang
*** 🐁 [[./lang/applescript][applescript]]: editing [[https://en.wikipedia.org/wiki/AppleScript][AppleScript]] files using [[https://github.com/AjaiKN/apples-mode][apples-mode]]
...
*** 🐁 [[./lang/ledger+hledger][ledger+hledger]]: editing [[https://www.ledger-cli.org/][ledger]]/[[https://hledger.org/][hledger]] plain-text accounting files
...
*** 🏢 [[./lang/mediawiki][mediawiki]]: editing and browsing MediaWiki pages (e.g. Wikipedia, Wiktionary) from Emacs
Fork of [[https://github.com/hexmode/mediawiki-el][mediawiki-el]] with extra commands/keybindings and various fixes.
...
*** [[./lang/vue][vue]]: [[https://vuejs.org/][Vue]] single-file components
Uses [[https://github.com/8uff3r/vue-ts-mode][vue-ts-mode]] with tree-sitter, or falls back to ~web-mode~ otherwise.
** :os
...
*** 🐁 [[./term/eshell-extra][eshell-extra]]: extra eshell utilities
Adds ~$~, ~_~, and ~OUTPUT~ variable aliases for referencing the last command's output (as text, as a list, or as a file). From [[https://github.com/howardabrams/hamacs][hamacs]].
*** ⭐🐛 [[./term/ghostel][ghostel]]: support for the [[https://github.com/dakra/ghostel][ghostel]] terminal emulator
...
*** 🐁 [[./tools/guix][guix]]: integration with [[https://guix.gnu.org/][GNU Guix]]
Adds Guix profile directories to ~exec-path~/~PATH~, and a helper for attaching a buffer to a Guix REPL.
*** 🚧🏢 [[./tools/llm-extra][llm-extra]]: stuff related to large language models
Meant to be used in addition to Doom's ~:tools llm~ module.
*** [[./tools/mason][mason]]: install LSPs and other external dependencies using [[https://github.com/deirn/mason.el][mason.el]] (the same registry as [[https://github.com/mason-org/mason.nvim][mason.nvim]])
Run ~doom +mason sync~ to install dependencies for the Doom modules you have enabled.

Flags:
**** +sync: automatically install the relevant Mason packages every time you run ~doom sync~
*** 🐁 [[./tools/mise][mise]]: integration for [[https://mise.jdx.dev/][mise]] (dev tool/environment version manager)
Puts mise's shims on ~PATH~ / ~exec-path~, and automatically trusts mise/tool-version config files when you save them.
...
*** 🐁 [[./tools/vim-file-locals][vim-file-locals]]: apply Vim-style modelines using [[https://github.com/abougouffa/emacs-vim-file-locals][vim-file-locals]]
** :ui
...
*** 🐁 [[./ui/mini-frame][mini-frame]]: show the minibuffer in a centered child frame using [[https://github.com/muffinmad/emacs-mini-frame][mini-frame]]
