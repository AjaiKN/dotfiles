...
;;;###autoload
(defalias '+nav-flash-delayed-blink-cursor-a #'+nav-flash-delayed-blink-cursor-h)
...
(defun +nav-flash/highlight-cursor (&rest _)
  (interactive)
  (unless (or (bound-and-true-p so-long-minor-mode)
              (minibufferp)
              (not (display-graphic-p)))
