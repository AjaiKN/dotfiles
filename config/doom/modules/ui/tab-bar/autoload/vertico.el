;;; ui/tab-bar/autoload/vertico.el -*- lexical-binding: t; -*-
;;;###if (modulep! :completion vertico)

;; copied from completion/vertico/autoload/workspaces.el

(defun +tab-bar+vertico--workspace-buffer-state ()
  (let ((preview
                       ...
                       (with-selected-window win
                         (cond
                          ...
                          ((buffer-live-p orig-buf)
                           ...
                           ;; CHANGED
                           (mapc #'+workspace-remove-buffer cleanup-buffers)))))))))
           #'ignore)))
    (lambda (action cand)
      (funcall preview action cand))))

(defun +tab-bar+vertico--workspace-generate-sources ()
  ...
  (let* ((active-workspace (+workspace-current-name))
    ...
    (mapcar (lambda (name)
              ...
              `(:name     ,name
                ...
                :category buffer
                :state    +tab-bar+vertico--workspace-buffer-state
                :items    ,(lambda ()
...
;;;###autoload
(defun +tab-bar+vertico/switch-workspace-buffer (&optional force-same-workspace)
...
buffer will be opened in the current workspace instead."
  (interactive "P")
  (when-let (buffer (consult--multi (+tab-bar+vertico--workspace-generate-sources)
                                    :require-match
...
;;;###autoload
(defun +tab-bar+vertico/embark-open-in-new-workspace (file)
