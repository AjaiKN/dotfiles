...
;;;###autoload
(defun +multiple-cursors/click-add-cursor (*click)
  "Add a fake cursor at point, then move point to clicked position."
  ;; https://wilkesley.org/~ian/xah/emacs/emacs_mouse_wheel_config.html
  (interactive "e")
...
;;;###autoload (autoload '+multiple-cursors/evil-mc-make-cursor-move-next-line "editor/multiple-cursors/autoload/evil-mc" nil t)
(evil-define-command +multiple-cursors/evil-mc-make-cursor-move-next-line (count)
  "Add a fake cursor at point, then move down.

If we were in insert state before, put us back in insert state
afterwards. Also, disable `line-move-visual', since I don't think
it's very useful here."
  :repeat ignore
  :evil-mc t
  (interactive "<c>")
  (let ((line-move-visual nil)
        (before-state evil-state))
    (when (eq evil-state 'insert)
      (let ((evil-move-cursor-back nil))
        (evil-normal-state)))
    (evil-mc-make-cursor-move-next-line count)
    (when (eq before-state 'insert)
      (call-interactively #'evil-insert-state))))
;;;###autoload (autoload '+multiple-cursors/evil-mc-make-cursor-move-prev-line "editor/multiple-cursors/autoload/evil-mc" nil t)
(evil-define-command +multiple-cursors/evil-mc-make-cursor-move-prev-line (count)
  "Add a fake cursor at point, then move up.
...
it's very useful here."
  ...
  (let ((line-move-visual nil)
    ...
    (when (eq evil-state 'insert)
      (let ((evil-move-cursor-back nil))
        (evil-normal-state)))
    (evil-mc-make-cursor-move-prev-line count)
    (when (eq before-state 'insert)
      (call-interactively #'evil-insert-state))))
