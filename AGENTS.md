...
- Adding a new tracked file/dir under an unfolded directory (e.g. `private/dot-home/.claude/`): move the real file into place in the repo (`mv`, not a manual `ln -s`), then run `install-files.sh` to create the actual symlink at the `$HOME` location — it detects the destination is now missing and links cleanly with no prompt. If the destination still exists (not yet moved out of the way), it prompts interactively with backup/adopt/trash/skip options; "adopt" does the move-then-link for you in one step, but for a non-interactive agent, doing the `mv` yourself first and then invoking the script is the reliable route.
