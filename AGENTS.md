...
- Emacs runs as persistent daemons via launchd, not started fresh per session. Two are configured:
  - `~/Library/LaunchAgents/dev.ajai.emacsdaemonserver.plist` — the main/GUI daemon, default `emacsclient` socket.
  - `~/Library/LaunchAgents/dev.ajai.emacsdaemonterm.plist` — a separate daemon for terminal clients, reached via `emacsclient --socket-name=term`.
